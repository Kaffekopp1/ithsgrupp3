...
<template>
  ...
  <div class="searchDiv" columns v-if="showSearch">
    ...
    <b-card-group class="searchDiv2">
      <b-card v-for="actor in ActorSeachedoutput" :header="actor.personName" tag="article"
        style="max-width: 15rem; min-height: 600px; min-width: 15rem" class="mb-2">
        <b-card-img v-if="actor.personIMG" :src="`https://image.tmdb.org/t/p/w500${actor.personIMG}`" alt="Image"
          top></b-card-img>
        <b-card-img v-else :src="`https://placehold.co/200x300/212529/FFF?text=?`" alt="Image" top></b-card-img>
        <div v-for="movie in actor?.movies">
          {{ movie.movieName }}
        </div>
        <router-link :to="{ name: 'person', params: { personId: actor.personId } }">
  ...
  <b-container>
    <div>
      <b-form-input class="mr-sm-1" placeholder="Sök efter en skådespelare" v-model="ActorInput"
        required></b-form-input>
      <b-button variant="primary" class="my-2 my-sm-0" type="submit" @click="searchActor()">Sök</b-button>
    </div>

    <div class="d-flex justify-content-center align-items-center flex-column" v-if="personArr?.personName">
      <h1>{{ personArr.personName }}</h1>
      <img style="max-width: 20rem" :src="`https://image.tmdb.org/t/p/w500${personArr.personImg}`" />
      <p>
        {{ personArr.personBorn }}
      </p>
      <p>Filmer med {{ personArr.personName }}:</p>
      <b-card-group>
        <b-card v-for="movie in personMovies" :title="movie.movieName" img-alt="Image"
          :img-src="`https://image.tmdb.org/t/p/w500${movie.moviePoster}`" img-top tag="article"
